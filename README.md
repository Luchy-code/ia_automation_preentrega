...
##  Propósito
Registrar diariamente el valor de compra-venta del dólar oficial y gestionar errores de API en Google Sheets.
...
*   **Gestor de Errores:**
    *   Valida la existencia de la propiedad `error`.
    *   Si es `True`, registra el incidente (Fecha, Hora, Tipo de Error, Prioridad Alta) en la hoja `error_log` del documento **Pre_entrega-sheet** y envía una notificación.
...
*   **Variables Registradas:** Se capturan y formatean:
    *   Fecha
    *   Hora (formato HH:mm:ss)
    *   Valor Dólar Compra
    *   Valor Dólar Venta
    *   Estado de confirmación: `"ok"` (encabezado del cuadro de excel).
...
*   **Happy path:** El cuerpo del mensaje incluye un cuadro informativo en *formato HTML* con el resumen de la operación diaria.
*   **Failure path:** Se reporta el detalle del error técnico.
